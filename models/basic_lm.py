...
class BasicNeuralLM:
    ...
    def build_placeholders(self):
        self.train_input = tf.placeholder(tf.float32, shape=[self.conf.batch_size, self.conf.num_steps])
        self.train_labels = tf.placeholder(tf.float32, shape=[self.conf.batch_size, 1]) # list of word ids
        ...
        with tf.device(self.conf.device):
            with tf.name_scope('embed'):
                if self.conf.trainable == False:
                    self.embed_matrix = tf.Variable(tf.random_uniform([len(self.data.V), self.conf.embed_dim], -1.0, 1.0),
                                                    trainable=False,
                                                    name='embed_matrix')
                else:
                    self.embed_matrix = tf.Variable(tf.constant(0.0, shape=[len(self.data.V), self.conf.embed_dim]),
                                                    name='embed_matrix')
        ...
        with tf.device(self.conf.device):
            with tf.name_scope('loss'):
                ...
                if self.conf.direct_connections == True:
                    W = tf.Variable(tf.random_normal([self.conf.embed_dim * self.conf.num_steps, len(self.data.V)], -1.0, 1.0))
                else:
                    W = tf.Variable(np.zeros([self.conf.embed_dim * self.conf.num_steps, len(self.data.V)]), trainable=False)
